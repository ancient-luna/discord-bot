...
registerFont('assets/PaybAck.ttf', { family: 'PaybAck' })
registerFont('assets/FRAHV.TTF', { family: 'FRAHV' })
...
module.exports.run = async (client, message, args, config) => {
  ...
  fs.readFile('assets/template.png', (err, data) => {
    if(err) throw err;
    ...
    loadImage(data).then(async (image) => {
        ...
        row.roles.forEach((r, index) => {
          ...
          if(role){
            ...
            if(endX > beginX){
              ...
              roundRect(ctx, beginX, beginY-25, length+(padding*2), parseInt(fontSize)+7, 22, true, false);
      ...
      await loadImage(message.member.displayAvatarURL({ format: "png" })).then(avatar => {
        let avatarX = 310;
        let avatarY = 310;
