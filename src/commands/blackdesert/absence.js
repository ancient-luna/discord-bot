...
module.exports.run = async (Client, message, args) => {
    ...
    const elderHall = '1060992670035619931'

    if (!elderHall.includes(message.channel.id)) return message.channel.send(`*You can't send a vacancy letter from here, go back to <#1060992670035619931> to send!*`).then((msg) => {
    ...
    if (!args[0]) return message.channel.send(`*You can't let the vacancy letter be an empty-ink letter*`).then((msg) => {
        setTimeout(() => msg.delete().catch((e) => {}), 5000);
        setTimeout(() => message.delete().catch((e) => {}));
    }).catch((err) => {
        throw err;
    ...
    if (message.channel.id === elderHall) {
